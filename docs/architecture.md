...
|----------|---------------------------------------------|------|
| View     | [`MainWindow`](../src/views/main_window.py) | Управляет панелями портов/консоли, статус-баром и карточками секундомера, подписывается на `ViewModelFactory`. |
| View     | [`StopwatchWidget`](../src/views/widgets/stopwatch_widget.py) + [`StopwatchWindow`](../src/views/stopwatch_window.py) | Общий UI компонентов секундомера: правый блок и отдельное окно, синхронизируются через ViewModel. |
...
| Utils    | [`StopwatchService`](../src/utils/stopwatch.py) | Qt-сервис с форматированием `dd hh:mm:ss.mmm`, испускает состояние в ViewModel. |
...
4. [`StopwatchViewModel`](../src/viewmodels/stopwatch_viewmodel.py) подписывается на [`StopwatchService`](../src/utils/stopwatch.py) и ретранслирует `time_changed/state_changed` в `MainWindow` (индикатор статус-бара) и [`StopwatchWidget`](../src/views/widgets/stopwatch_widget.py) / [`StopwatchWindow`](../src/views/stopwatch_window.py).
5. Изменения темы/языка инициируют события `theme_manager.theme_changed` и `translator.language_changed`, на которые подписаны `IconCache`, `ConsolePanelView`, `StopwatchWidget`, `QuickBlocksPanel` и другие View-классы.
6. Quick Blocks редактируются во View (`QuickBlocksPanel`), затем вызывают методы [`QuickBlocksRepository`](../src/utils/quick_blocks_repository.py) для записи YAML; при перечитывании используется схема [`QuickBlocksDocument`](../src/utils/quick_blocks_schema.py) для валидации.
